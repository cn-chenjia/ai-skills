...
- 新需求先完成 propose 并等待用户确认；确认后调用 initialize-requirement.mjs 建账并记录确认人，随后由用户明确选择“确认并立即实施当前需求”“确认并暂停当前需求”或“确认并结束本次会话，稍后新会话创建下一个需求”。后两种选择保持 \`not-started\`，不得进入 apply、验证、评审、归档或 finish。
- 仅“确认并立即实施当前需求”才调用 prepare-workspace.mjs；当前工作区未被其他 active 需求占用时可登记为专属工作区，不要求额外新建 worktree。
