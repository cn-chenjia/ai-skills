...
- 未明确激活小七时，不得接管普通需求；不得把普通消息或“确认/继续”等短消息解释为小七续接。
- 只有当前会话已明确激活小七且存在对应需求账本时，小七才接管后续用户消息。
...
- 只有用户明确输入“退出小七”，或在 proposal 确认后明确选择“确认并结束本次会话”才解除会话接管。
- 一个小七会话只聚焦一个当前需求；代码可在多个 worktree 并存，但不得在同一会话交错讨论或自动推进多个需求。
- 上下文被压缩或恢复后，先重新读取小七技能和当前需求账本，再继续原流程。
- 新需求先完成 propose 并等待用户确认；确认结果和交付状态由 SQLite 控制平面记录，随后由用户明确选择“确认并立即实施当前需求”“确认并暂停当前需求”或“确认并结束本次会话”。后两种选择保持 \`not-started\`，不得进入 apply、验证、评审、归档或 finish。
- 仅“确认并立即实施当前需求”才通过控制平面准备工作区；当前工作区未被其他 active 需求占用时可登记为专属工作区，不要求额外新建 worktree。
- 普通失败先自动修复并重试，同一错误第 3 次失败才 blocked；高风险、权限或人工确认错误可以立即 blocked。
- 到达 ready 后停止自动流程，等待用户选择创建 PR、合并或保留分支，再通过控制平面执行归档和 finish。
- 最终交付与归档证据齐全后由控制平面记录 closed，不能只口头宣称 closed。
